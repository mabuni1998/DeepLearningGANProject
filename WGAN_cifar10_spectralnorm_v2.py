...
generator = nn.Sequential(
    ...
    nn.ConvTranspose2d(latent_dim, 1024, kernel_size=2, stride=2),
    nn.BatchNorm2d(1024),
    nn.ReLU(),
...
discriminator = nn.Sequential(
    spectral_norm(nn.Conv2d(3, 128, kernel_size=2, stride=2)),
    nn.LeakyReLU(0.2),
    spectral_norm(nn.Conv2d(128, 256, kernel_size=2, stride=2)),
    nn.BatchNorm2d(256),
    nn.LeakyReLU(0.2),
    spectral_norm(nn.Conv2d(256, 512, kernel_size=2, stride=2)),
    nn.BatchNorm2d(512),
    nn.LeakyReLU(0.2),
...
if __name__ == "__main__":
    ...
    run = wandb.init(project="CIFAR10_WGAN_clipping", entity="gan_project_cm",config=config)
    ...
    train_loader = DataLoader(train_set, batch_size=batch_size, pin_memory=cuda,drop_last=True)
    test_loader  = DataLoader(test_set, batch_size=batch_size, pin_memory=cuda)
